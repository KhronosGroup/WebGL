...
var glEnums = {};
var re = /const\s+GLenum\s+(\w+)\s+=\s+0x([0-9a-f]+);/i;

for (var ii = 0; ii < webGLIdl.length; ++ii) {
  var matches = webGLIdl[ii].match(re);
  if (matches) {
    var value = parseInt(matches[2], 16);
    glEnums[value] = matches[1];
  }
}
